...
package cva6_cheri_pkg;
    ...
    /* Capability default root capability and null capabilities for capability registers */
   ...
   function automatic cap_mem_t set_cap_mem_addr_unsafe (capw_t cap, addrw_t addr);
        cap_mem_t ret = cap;
        ret.addr = addr;
        return ret;
    endfunction

    function automatic cap_mem_t set_cap_mem_addr_inc (capw_t cap, addrw_t inc);
        cap_mem_t ret = cap;
        addrw_t addr = get_cap_mem_addr(cap);
        ret = set_cap_mem_addr_unsafe(cap, addr + $signed(inc));
        return ret;
    endfunction
