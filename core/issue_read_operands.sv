...
#(
    ...
    output logic [CVA6Cfg.NrIssuePorts-1:0][CVA6Cfg.REGLEN-1:0] rs1_forwarding_o,
  ...
  typedef struct packed {
    logic none, load, store, alu, alu2, ctrl_flow, mult, csr, fpu, fpu_vec, cvxif, accel, aes, clu;
  } fus_busy_t;
  ...
  logic [CVA6Cfg.NrIssuePorts-1:0][CVA6Cfg.REGLEN-1:0] operand_a_regfile, operand_b_regfile;
  ...
  logic [CVA6Cfg.NrIssuePorts-1:0] clu_valid_n, clu_valid_q;
  ...
  if (CVA6Cfg.CheriPresent) begin : gen_cheri_pcc_checks
    ...
    always_comb begin : pcc_bounds
      ...
      for (int unsigned i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
        ...
        next_pc_addr = issue_instr_i[i].pc + next_pc_off;
        if (!issue_instr_i[i].ex.valid) begin
          if((cva6_cheri_pkg::addrw_t'(signed'(issue_instr_i[i].pc)) < pcc_base) || ({0,cva6_cheri_pkg::addrw_t'(signed'(next_pc_addr))} > pcc_top)) begin
  ...
  for (genvar i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
    always_comb begin : forwarding_operand_select
      ...
      if (issue_instr_i[i].use_zimm) begin
        ...
        fu_data_n[i].operand_a[CVA6Cfg.XLEN-1:0] = {{CVA6Cfg.XLEN - 5{1'b0}}, issue_instr_i[i].rs1[4:0]};
      end
  ...
  for (genvar i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
    ...
    assign operand_a_regfile[i] = (CVA6Cfg.FpPresent && is_rs1_fpr(
        issue_instr_i[i].op
    )) ? {{CVA6Cfg.REGLEN - CVA6Cfg.FLen{1'b0}}, fprdata[0]} : rdata[i*OPERANDS_PER_INSTR+0];
    assign operand_b_regfile[i] = (CVA6Cfg.FpPresent && is_rs2_fpr(
        issue_instr_i[i].op
    )) ? {{CVA6Cfg.REGLEN - CVA6Cfg.FLen{1'b0}}, fprdata[1]} : rdata[i*OPERANDS_PER_INSTR+1];
  ...
  always_comb begin
    ...
    if (CVA6Cfg.SuperscalarEn) begin
      if (issue_instr_i[1].fu == CTRL_FLOW) begin
        pc_n                  = cva6_cheri_pkg::set_cap_reg_address(pcc[1], issue_instr_i[1].pc, cva6_cheri_pkg::get_cap_reg_meta_data(pcc_q))
    ...
    if (issue_instr_i[0].fu == CTRL_FLOW) begin
      pc_n                  = cva6_cheri_pkg::set_cap_reg_address(pcc[0], issue_instr_i[0].pc, cva6_cheri_pkg::get_cap_reg_meta_data(pcc_q))
