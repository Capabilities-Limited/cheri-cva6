...
#(
  ...
  if (CVA6Cfg.ZKN && CVA6Cfg.RVB) begin : zkn_gen_block
    ...
    for (i = 0; i < (CVA6Cfg.XLEN / 8); i++) begin : brev8_xperm8_gen
      ...
      assign xperm8_result[i << 3 +: 8] = ({24'b0, operand_b[i << 3 +: 8]} < (CVA6Cfg.XLEN / 8)) ? operand_a[operand_b[i << 3 +: 6] << 3 +: 8] : 8'b0;
    ...
    for (q = 0; q < (CVA6Cfg.XLEN / 4); q++) begin : xperm4_gen
      ...
      assign xperm4_result[q << 2 +: 4] = ({28'b0, operand_b[q << 2 +: 4]} < (CVA6Cfg.XLEN / 4)) ? operand_a[{2'b0, operand_b[q << 2 +: 4]} << 2 +: 4] : 4'b0;
    end
