...
#(
  ...
  always_comb begin
    for (int i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
      ...
      if (commit_pcc_ex[i].valid) commit_instr_o[i].ex = commit_pcc_ex[i];
    end
  end
  ...
  function automatic exception_t [CVA6Cfg.NrIssuePorts-1:0] check_pcc_exceptions(
      ...
      input logic [1:0][CVA6Cfg.PCLEN-1:0] pccs, input logic debug_mode);
    ...
    for (int unsigned i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
      ...
      pcc_cur = cva6_cheri_pkg::set_cap_reg_flags(pccs[instr[i].pcc_gen], instr[i].int_mode);
      ...
      {next_pc_carry, next_pc_addr} = {1'b0, instr[i].pc} + {1'b0, next_pc_off};
      ...
      if (!instr[i].valid || debug_mode) pcc_ex[i].valid = 1'b0;
    end
