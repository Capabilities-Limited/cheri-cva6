...
#(
  ...
  always_comb begin : instr_interface
    ...
    if ((enable_translation_i || enable_g_translation_i)) begin
      ...
      if (itlb_lu_hit) begin
        ...
        end else if (((!pmp_instr_allow && !CVA6Cfg.RVFI_DII) || (!rvfii_instr_addr_allowed && CVA6Cfg.RVFI_DII))) begin
          icache_areq_o.fetch_exception.cause = riscv::INSTR_ACCESS_FAULT;
          icache_areq_o.fetch_exception.valid = 1'b1;
          if (CVA6Cfg.TvalEn)
            icache_areq_o.fetch_exception.tval = CVA6Cfg.XLEN'(icache_areq_i.fetch_vaddr);
    ...
    if ((!match_any_execute_region && !ptw_error) || (!(enable_translation_i || enable_g_translation_i) && ((!pmp_instr_allow && !CVA6Cfg.RVFI_DII) || (!rvfii_instr_addr_allowed && CVA6Cfg.RVFI_DII)))) begin
      icache_areq_o.fetch_exception.cause = riscv::INSTR_ACCESS_FAULT;
      icache_areq_o.fetch_exception.valid = 1'b1;
      if (CVA6Cfg.TvalEn)  //To confirm this is the right TVAL 
  ...
  assign rvfii_addr_allowed = config_pkg::range_check(64'h8000_0000, 64'h000800000, (en_ld_st_translation_i || en_ld_st_g_translation_i) ? {{64 - CVA6Cfg.PLEN{1'b0}}, icache_areq_o.fetch_paddr} : lsu_vaddr_q);
