...
module id_stage #(
  ...
  logic              [ariane_pkg::SUPERSCALAR:0]       int_mode_decode_o;
  logic              [ariane_pkg::SUPERSCALAR:0]       int_mode_decode;
  logic                                                int_mode_d;
  logic                                                int_mode_q;
  //logic                                                int_mode_reset_d;
  //logic                                                int_mode_reset_q;

  for (genvar i = 0; i <= ariane_pkg::SUPERSCALAR; i++) begin
    assign int_mode_decode[i] = (i==0) ? int_mode_q : int_mode_decode_o[i-1];
                        //((is_mispredict_i) ? int_mode_resolved_branch_i : int_mode_q)
                        //(int_mode_reset_q ? int_mode_issue_i : int_mode_q)                 
  end
  ...
  always_comb begin
    ...
    if (is_mispredict_i) int_mode_d = int_mode_resolved_branch_i;  // Probably the wrong priority, but flush_i also happens on mispredict.
      ...
      for (int i = 0; i <= ariane_pkg::SUPERSCALAR; i++) begin
